...
idLine<-paste(metagenomes$MG.RAST.ID,collapse = ' ')
seedline<-paste('sleep 6 \n sbatch getMGRAST.loop.seed.sh',webkey,idLine)
fsubline<-paste('sleep 6 \n sbatch getMGRAST.loop.fsub.sh',webkey,idLine)
koline<-paste('sleep 6 \n sbatch getMGRAST.loop.ko.sh',webkey,idLine)
refseqline<-paste('sleep 6 \n# sbatch getMGRAST.loop.refseq.sh',webkey,idLine)
frefseqline<-paste('sleep 6 \n# sbatch getMGRAST.loop.frefseq.sh',webkey,idLine)
lines<-c('#!/bin/bash',seedline,fsubline,koline,refseqline,frefseqline)
writeLines(lines,'submit.sh')
system('chmod a+x submit.sh')
```
```
{r make.sango.scripts,echo=FALSE}
lines<-c('#!/bin/bash',sapply(metagenomes$MG.RAST.ID,function(.x)paste0('sleep 6 \n sbatch getMGRAST.sh ',webkey,' ',.x)))
writeLines(lines,'submit.sh')
system('chmod a+x submit.sh')
```
