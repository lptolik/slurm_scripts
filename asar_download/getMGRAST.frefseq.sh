...
srun curl  -H "auth: $key" -H 'Accept-Encoding: gzip,deflate' "http://api.metagenomics.anl.gov/1/annotation/similarity/$mgid?source=RefSeq&type=function&identity=60&length=15" -o "$mgid.frefseq"

# copy our result back to bucket. We need to use "scp"
# to copy the data back as bucket isn't writable directly. 
scp "$mgid.frefseq" deigo:"$datdir/$mgid.frefseq"
